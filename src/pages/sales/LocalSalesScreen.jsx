...
const LocalSalesScreen = () => {
  ...
  useEffect(() => {
    if (data) {
      setSales(data?.localsales?.sales?.data || []);
      setPagination({
        current_page: data?.localsales?.sales?.current_page,
        last_page: data?.localsales?.sales?.last_page,
        total: data?.localsales?.sales?.total,
        from: data?.localsales?.sales?.from,
        to: data?.localsales?.sales?.to
      });
      setStats(data?.localsales?.stats || {});
      }
  ...
  const loadSales = async (page = 1) => {
    try {
      ...
      dispatch(fetchApiData({ url: '/api/local-sales', itemKey: 'localsales', method: 'GET', params }));
  ...
  const handleDeleteSale = async (saleId) => {
    try {
      const response = await ApiService.delete(`/api/local-sales/${saleId}`);
      if (response.success) {
  ...
  const handleCancelSale = async (saleId, reason) => {
    try {
      const response = await ApiService.put(`/api/local-sales/${saleId}/cancel`, { description: reason });
      if (response.success) {
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      const response = await ApiService.post(`/api/local-sales/${saleId}/payment`, {
        ...
        <div className="col-12">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="d-flex gap-2">
              ...
              <button  onClick={()=>navigate('/localsales/create')} className="btn btn-primary">
                    ...
                    <tr key={index}>
                      ...
                      <td className="px-4">
                        <div className="btn-group" role="group">
                          <Link to={`/localsales/${sale.id}`} className="btn btn-sm btn-outline-primary" title={intl.formatMessage({id: "sales.view"})}>
                            <i className="pi pi-eye"></i>
                          </Link>
                          {sale.status !== 'cancelled' && (
                            <Link href={`/localsales/${sale.id}/edit`} className="btn btn-sm btn-outline-warning" title={intl.formatMessage({id: "sales.edit"})}>
...
export default LocalSalesScreen;