...
const LocalSalesCreateScreen = () => {
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true);
      const response = await ApiService.get("/api/local-sales/create-data");
  ...
  const loadCategories = async () => {
    ...
    try {
      const response = await ApiService.get(
        `/api/local-sales/categories/${selectedStock}`
      );
      if (response.success) {
  ...
  const searchClients = async (search) => {
    ...
    try {
      ...
      const response = await ApiService.get("/api/local-sales/clients/search", {
        search,
      });
  ...
  const searchProducts = async () => {
    ...
    try {
      ...
      const response = await ApiService.get(
        "/api/local-sales/products/search",
        params
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await ApiService.post("/api/local-sales/store", saleData);
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          navigate("/localsales/"+response.data.id);
        }, 1000);
      } else {
        showToast(
          "error",
...
export default LocalSalesCreateScreen;
